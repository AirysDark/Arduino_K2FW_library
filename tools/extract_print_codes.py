...
def main():
    ...
    codes = {}
    def ensure(code_key, code_str):
        return codes.setdefault(code_key, {
            "code": code_str,
            "meaning": KNOWN.get(code_str, ""),
            "occurrences": [],
            "evidence_files": set(),
            "used_by_macros": [],
            "notes": ""
    ...
    for f in iter_files(dump_root, paths_json=paths_json):
        ...
        for i, ln in enumerate(lines, start=1):
            for m in CODE_RE.finditer(ln):
                ...
                rec = ensure(key, code)
                if len(rec["occurrences"]) < 50:
                    before = lines[i-2].strip() if i-2 >= 0 else ""
                    after  = lines[i].strip() if i < len(lines) else ""
                    rec["occurrences"].append({
                        "file": rel,
                        "line": i,
                        "context": {"before": before[:180], "line": ln.strip()[:220], "after": after[:180]}
                    })
                rec["evidence_files"].add(rel)

    # Link to GC macros
    if macro_map:
        for mk, mlines in macro_map.items():
            blob = "\n".join(mlines)
            for m in CODE_RE.finditer(blob):
                ...
                key = norm_key(letter, num)
                rec = ensure(key, code)
                if mk not in rec["used_by_macros"]:
                    rec["used_by_macros"].append(mk)

    out = {"meta": {"dump_root": str(dump_root),
                     "paths_json": str(paths_json) if paths_json else "",
                     "gcode_macros_json": str(gcode_macros_json) if gcode_macros_json else ""},
            "codes": {}}
    for k, rec in codes.items():
        rec["evidence_files"] = sorted(list(rec["evidence_files"]))
        rec["used_by_macros"] = sorted(rec["used_by_macros"])
        if rec["code"].startswith("M28"):
            rec["notes"] = "Often starts SD/virtual-SD write/upload; paired with M29. Firmware-specific."
        elif rec["code"].startswith("M29"):
