...
class HomingMove:
    ...
    def handle_force_stop(self):
        # Vendor safety hook. Must NEVER crash klippy.
        toolhead = self.printer.lookup_object('toolhead')

        # Do NOT hard-crash the reactor if vendor commands are missing.
        try:
            toolhead._handle_shutdown()
            toolhead.reactor.pause(toolhead.reactor.monotonic() + 1.0)
        except Exception as e:
            logging.warning("handle_force_stop: shutdown sequence skipped: %s", e)
        ...
        try:
            ...
            try:
                ...
                logging.warning("handle_force_stop: MOTOR_CHECK_PROTECTION_AFTER_HOME failed: %s", e)
            try:
                gcode.run_script_from_command("MOTOR_STALL_MODE DATA=2")
            except Exception as e:
                logging.warning("handle_force_stop: MOTOR_STALL_MODE failed: %s", e)
        except Exception as e:
            logging.warning("handle_force_stop: gcode object unavailable: %s", e)
            ...
            if trigger_time > 0.:
                ...
                if name == "z":
                    ...
                    try:
                        ...
                        logging.warning("MOTOR_STALL_MODE failed in z exception path: %s", e)
